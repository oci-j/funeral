...
public class MongoManifestStorage implements ManifestStorage {
    ...
    public Manifest findByRepositoryAndDigest(String repositoryName, String digest) {
        return Manifest.find("repository_name = ?1 and digest = ?2", Sort.by("updated_at", Sort.Direction.Descending), repositoryName, digest).firstResult();
    }

    @Override
    public Manifest findByRepositoryAndTag(String repositoryName, String tag) {
        return Manifest.find("repository_name = ?1 and tag = ?2", Sort.by("updated_at", Sort.Direction.Descending), repositoryName, tag).firstResult();
    }

    @Override
    public List<Manifest> findByRepository(String repositoryName) {
        return Manifest.find("repository_name", Sort.by("updated_at", Sort.Direction.Descending), repositoryName).list();
    }
    ...
    public List<Manifest> findBySubjectDigest(String repositoryName, String subjectDigest) {
        return Manifest.find("repository_name = ?1 and subject.digest = ?2", Sort.by("updated_at", Sort.Direction.Descending), repositoryName, subjectDigest).list();
    }

    @Override
    public List<Manifest> findBySubjectDigestAndArtifactType(String repositoryName, String subjectDigest, String artifactType) {
        return Manifest.find("repository_name = ?1 and subject.digest = ?2 and artifact_type = ?3", Sort.by("updated_at", Sort.Direction.Descending), repositoryName, subjectDigest, artifactType).list();
    }
    ...
    public List<String> findTagsByRepository(String repositoryName, String last, int limit) {
        ...
        if (last == null) {
            manifests = Manifest.find("repository_name = ?1 and tag != ?2", Sort.by("updated_at", Sort.Direction.Descending), repositoryName, null)
                    .page(0, limit)
                    .list();
        } else {
            manifests = Manifest.find("repository_name = ?1 and tag != ?2 and tag > ?3", Sort.by("updated_at", Sort.Direction.Descending), repositoryName, null, last)
                    .page(0, limit)
                    .list();
        }
    ...
    public long countByRepository(String repositoryName) {
        return Manifest.count("repository_name = ?1 and tag != null", repositoryName);
    }
