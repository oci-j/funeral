...
/**
 * Resource for uploading and analyzing Docker tar files created by 'docker save' command. Automatically extracts image
 * metadata and stores it in the registry.
 */
@Path(
...
public class DockerTarResource {
    ...
    public Response uploadDockerTar(
            @FormParam(
                "file"
            )
    ...
    private Response processSingleTarFile(
        ...
        try {
            ...
            tempFile = saveToTempFile(
                    fileInputStream
            );

            // Check if it's a zip file and extract tar if needed
            tarFile = extractTarFromZipIfNeeded(
                    tempFile
            );

            // Parse the tar file
            TarParseResult result = parseDockerTar(
                    tarFile != null ? tarFile : tempFile
            );
            ...
            log.error(
                    "Failed to process Docker tar file",
                    e
            );
            return Response.status(
                    Response.Status.BAD_REQUEST
    ...
    /**
     * Extracts tar file from zip archive if the uploaded file is a zip. Returns the extracted tar file or null if no
     * extraction was needed.
     */
    private File extractTarFromZipIfNeeded(
            File uploadedFile
    )
            throws IOException {
        ...
        try (
            ...
            if (signature[0] == 0x50 && signature[1] == 0x4B) {
                log.info(
                        "Detected zip file, extracting tar archive..."
                );
                return extractTarFromZip(
                        uploadedFile
                );
            }
        }
     ...
     */
    private File extractTarFromZip(
            File zipFile
    )
            throws IOException {
        File extractedTar = File.createTempFile(
        ...
        try (
            ...
            while ((entry = zis.getNextEntry()) != null) {
                ...
                if (entryName.endsWith(
                    ...
                    log.info(
                            "Successfully extracted tar file from zip"
                    );
                    return extractedTar;
                }
                zis.closeEntry();
    ...
    /**
     * Parses Docker tar file and extracts image metadata and actual blob content. Handles both plain tar and
     * gzip-compressed tar files.
     */
    private TarParseResult parseDockerTar(
            File tarFile
    )
            throws IOException {
        ...
        log.info(
                "Parsing Docker tar file: {} (size: {} bytes)",
                tarFile.getName(),
                tarFile.length()
        );
        ...
        byte[] signature = new byte[4];
        try (
                FileInputStream checkFis = new FileInputStream(
                        tarFile
                )) {
            bytesRead = checkFis.read(
                    signature
            );
            ...
            if (bytesRead >= 2 && (signature[0] & 0xFF) == 0x1f && (signature[1] & 0xFF) == 0x8b) {
                isGzip = true;
                log.info(
                        "Detected gzip signature: {}{} (0x1f 0x8b)",
                        String.format(
                                "%02X",
                                signature[0] & 0xFF
                        ),
            ...
            // But some files may have 0x28, 0xB5, 0x2F, 0xFD (little-endian byte order)
                ...
                isZstd = true;
                log.info(
                        "Detected Zstandard signature: {}{}{}{} (big-endian)",
                        String.format(
                                "%02X",
                                signature[0] & 0xFF
                        ),
            ...
            // Check for little-endian Zstandard signature
                ...
                isZstd = true;
                log.info(
                        "Detected Zstandard signature: {}{}{}{} (little-endian)",
                        String.format(
                                "%02X",
                                signature[0] & 0xFF
                        ),
                ...
                log.info(
                        "File signature: {}{}{}{} (not recognized as gzip or zstd)",
                        String.format(
                                "%02X",
                                signature[0] & 0xFF
                        ),
                ...
                log.info(
                        "File signature: {}{} (too short, not recognized)",
                        String.format(
                                "%02X",
                                signature[0] & 0xFF
                        ),
                ...
                log.warn(
                        "Could not read file signature, file might be too small or corrupted"
                );
            }
        }
        ...
        if (isZstd) {
            log.info(
                    "Detected Zstandard compressed tar file (.tar.zst), decompressing..."
            );
            try (
                    FileInputStream fis = new FileInputStream(
                            tarFile
                    ...
                    ZstdCompressorInputStream zstdStream = new ZstdCompressorInputStream(
                            bis
                    )) {
                processTarStream(
                        zstdStream,
                ...
                log.error(
                        "Failed to decompress Zstandard stream",
                        e
                );
                throw new IOException(
                        "Failed to decompress Zstandard stream: " + e.getMessage(),
                        e
                );
            }
        }
            ...
            log.info(
                    "Detected gzip compressed tar file, decompressing..."
            );
            try (
                    FileInputStream fis = new FileInputStream(
                            tarFile
                    ...
                    GzipCompressorInputStream gzipStream = new GzipCompressorInputStream(
                            bis
                    )) {
                processTarStream(
                        gzipStream,
                ...
                throw new IOException(
                        "Failed to decompress gzip stream: " + e.getMessage(),
                        e
                );
            }
        }
        ...
        log.info(
                "Finished processing tar. blobContentMap has {} entries",
                blobContentMap.size()
        );
        ...
        for (ManifestInfo manifestInfo : manifestList) {
            ...
            for (String repoTag : manifestInfo.repoTags) {
                ...
                if (lastColonIndex != -1) {
                    tag = repoTag.substring(
                            lastColonIndex + 1
                    );
                    String repoPath = repoTag.substring(
                            0,
                            lastColonIndex
                    );
                ...
                log.debug(
                        "Parsed repoTag '{}': repository='{}', tag='{}'",
                        repoTag,
                        repositoryName,
                        tag
                );

                result.repositories.add(
                        repositoryName
                );
     ...
     */
    private void processTarStream(
            InputStream inputStream,
            TarParseResult result,
            List<ManifestInfo> manifestList,
            List<String> foundFiles,
            Map<String, byte[]> blobContentMap
    )
            throws IOException {
        int entryCount = 0;
        try (
            ...
            while ((entry = tarInput.getNextEntry()) != null) {
                ...
                log.info(
                        "Processing tar entry: {} (size: {} bytes)",
                        entryName,
                        entry.getSize()
                );
                ...
                if (entryName.equals(
                    ...
                    byte[] content = readEntryContent(
                            tarInput,
                            entry
                    );
                    String digest = extractDigestFromFilename(
                            entryName
                    );
                    ...
                    log.info(
                            "Read {} bytes for blob {} (full path: {}). Map size now: {}",
                            content.length,
                            digest,
                            fullPathKey,
                            blobContentMap.size()
                    ...
                    log.info(
                            "Read {} bytes for config blob {}. Map size now: {}",
                            content.length,
                            digest,
                            blobContentMap.size()
                    );
                    ...
                    log.info(
                            "Read {} bytes for layer blob {}. Map size now: {}",
                            content.length,
                            digest,
                            blobContentMap.size()
                    );
            ...
            log.info(
                    "Successfully processed {} entries from tar archive",
                    entryCount
            );

            if (!foundManifest) {
                log.warn(
                        "No manifest.json found in tar file. Available files:"
                );
                ...
                throw new IOException(
                        "No manifest.json found in tar file. Processed " + entryCount + " entries."
                );
            }
        }
        catch (IOException e) {
            log.error(
                    "IO error while processing tar stream at entry {}: {}",
                    entryCount,
                    e.getMessage()
            );
            throw new IOException(
                    "IO error while processing tar stream at entry " + entryCount + ": " + e.getMessage(),
                    e
            );
        }
        catch (Exception e) {
            log.error(
                    "Unexpected error while processing tar stream at entry {}: {}",
                    entryCount,
                    e.getMessage()
            );
            throw new IOException(
                    "Unexpected error while processing tar stream at entry " + entryCount + ": " + e.getMessage(),
                    e
            );
        }
    }
    ...
    /**
     * Using proper JSON parsing to parse manifest.json or index.json from Docker/OCI tar file. Supports both Docker
     * manifest.json format and OCI index.json format.
     */
    private List<ManifestInfo> parseManifestJson(
            InputStream inputStream
    )
            throws IOException {
        ...
        try {
            ...
            if (root.has(
                ...
                log.info(
                        "Detected OCI index.json format"
                );
                ...
                log.warn(
                        "OCI layout detected but not fully implemented yet. Please use Docker save format instead."
                );
            ...
            // Docker manifest.json format (array of manifest objects)
                ...
                log.info(
                        "Detected Docker manifest.json format"
                );
                for (JsonNode manifestNode : root) {
                    ...
                    if (manifestNode.has(
                        ...
                        JsonNode layersNode = manifestNode.get(
                                "Layers"
                        );
                        info.layers = new String[layersNode.size()];
                        for (int i = 0; i < layersNode.size(); i++) {
                            String layerFile = layersNode.get(
                                    i
                            ).asText();
                            // Remove file extension and extract just the digest
                            // Layer might be "abc...def.tar.gz" or "blobs/sha256/abc...def.tar.gz" or
                            // "abc...def.tar.zst"
                            String layerNoExt = layerFile.replace(
                                    ".tar.gz",
                                    ""
                            )
                                    .replace(
                            ...
                            String layerDigest = extractDigestFromFilename(
                                    layerNoExt
                            );
                            info.layers[i] = layerDigest;
                            log.debug(
                                    "Layer file: {}, extracted digest: {}",
                                    layerFile,
                                    layerDigest
                            );
                        }
                    ...
                    if (manifestNode.has(
                        ...
                        info.configSize = manifestNode.get(
                                "ConfigSize"
                        ).asLong();
                    }
                    ...
                    log.debug(
                            "Added manifest with {} layers",
                            info.layers != null ? info.layers.length : 0
                    );
                }
                log.info(
                        "Parsed {} manifests from tar file",
                        manifests.size()
                );
            }
            else {
                log.warn(
                        "Unknown JSON format in manifest/index file"
                );
            }
        }
        catch (Exception e) {
            ...
            throw new IOException(
                    "Failed to parse manifest.json: " + e.getMessage(),
                    e
            );
        }
     ...
     */
    private BlobInfo createBlobInfo(
            String digest,
            long size
    ) {
    ...
    private byte[] readEntryContent(
        ...
        log.debug(
                "Reading entry content for {} (expected size: {} bytes)",
                entry.getName(),
                entrySize
        );
        ...
        try {
            ...
            log.error(
                    "IO error reading entry {}: read {} of {} bytes - {}",
                    entry.getName(),
                    totalBytes,
                    entrySize,
                    e.getMessage()
        ...
        log.debug(
                "Successfully read {} bytes for entry {} (expected: {})",
                totalBytes,
                entry.getName(),
                entrySize
        );
     ...
     */
    private String extractDigestFromFilename(
            String filename
    ) {
    ...
    private void saveToStorage(
        ...
        log.info(
                "Saving {} repositories, {} manifests, and {} blobs to storage",
        ...
        for (TarManifest tarManifest : result.manifests) {
            ...
            if (existingManifest != null) {
                log.info(
                        "Tag '{}' already exists in repository '{}'. Deleting old manifest before saving new one.",
                        tarManifest.tag,
                        tarManifest.repository
                );
            ...
            String manifestContent = buildManifestJson(
                    tarManifest
            );
        ...
        for (BlobInfo blobInfo : result.blobs) {
            ...
            byte[] blobContent = result.blobData.get(
                    digestWithoutPrefix
            );
            ...
            if (blobContent == null) {
                ...
                blobContent = result.blobData.get(
                        fullPathKey
                );
                if (blobContent != null) {
                    log.debug(
                            "Found blob content using full path key: {}",
                            fullPathKey
                    );
                }
            }
            ...
            if (blobContent != null) {
                try {
                    ...
                    log.info(
                            "Stored {} bytes for blob: {} (verified digest: {})",
                            blobContent.length,
                            blobInfo.digest,
                            storedDigest
                    );
                    ...
                    log.error(
                            "Failed to store blob content for {}: {}",
                            blobInfo.digest,
                            e.getMessage(),
                            e
                    );
                ...
                log.warn(
                        "No content found for blob: {}. Available keys: {}",
                        blobInfo.digest,
                        result.blobData.keySet()
                );
            }
