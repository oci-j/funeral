...
public class BlobResourceHandler {
    ...
    public Response completeBlobUpload(
        ...
        log.info(
                "Completing blob upload for repository: {}, UUID: {}",
                repositoryName,
                uploadUuid
        );
        ...
        try {
            String actualDigest = storageService.storeBlob(
                    uploadStream,
                    expectedDigest
            );
    ...
    public Response completeBlobUploadChunkPatch(
        ...
        return this.completeBlobUploadChunkPatch(
                repositoryName,
                uploadUuid,
                "0_0",
                contentRange,
    ...
    @CommentPATCH
    @CommentPath(
        "/uploads/{uuid}/{index_and_start_bytes}"
    )
    public Response completeBlobUploadChunkPatch(
            @CommentPathParam(
                "name"
            ...
            String uploadUuid,
            @CommentPathParam(
                "index_and_start_bytes"
            )
            String indexAndStartBytes,
            @CommentHeaderParam(
                "Content-Range"
            )
            String contentRange,
        ...
        try {
            ...
            long startBytes = Long.parseLong(
                    split[1]
            );
            ...
            if (StringUtils.isNotBlank(
                ...
                try {
                    String[] contentRangeSplit = StringUtils.split(
                            contentRange,
                            '-'
                    );
            ...
            String location = "/v2/" + repositoryName + "/blobs/uploads/" + uploadUuid + "/" + (index + 1) + "_"
                    + endBytes;
            return Response.status(
                    202
            )
            ...
            log.error(
                    "completeBlobUploadChunkPatch failed WithResponseException",
                    e
            );
            ...
            log.error(
                    "completeBlobUploadChunkPatch failed",
                    e
            );
            return Response.status(
                    500
    ...
    public Response completeBlobUploadChunkGet(
        ...
        try {
            AbstractStorageService.CalculateTempChunkResult result = storageService.calculateTempChunks(
                    uploadUuid
            );
            int index = result.index();
            long bytesWritten = result.bytesWritten();

            String location = "/v2/" + repositoryName + "/blobs/uploads/" + uploadUuid + "/" + index + "_"
                    + bytesWritten;
            return Response.status(
                    204
            )
