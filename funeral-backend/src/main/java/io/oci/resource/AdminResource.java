...
public class AdminResource {
    ...
    public Response listUsers(
        ...
        List<UserResponse> users = userStorage.listAll()
                .stream()
                .map(
                        this::toUserResponse
                )
                    ...
                    .entity(
                            new ErrorResponse(
                                    java.util.List.of(
                                            new ErrorResponse.Error(
                                                    "CONFLICT",
                                                    "User already exists: " + request.username,
                                                    request.username
                                            )
                                    )
                            ).toJson()
    ...
    public Response deleteUser(
        ...
        User currentUser = userStorage.findByUsername(
                username
        );
        if (currentUser != null && currentUser.isAdmin()) {
            long adminCount = userStorage.listAll()
                    .stream()
                    .filter(
                            User::isAdmin
                    )
    ...
    public Response getUserPermissions(
        ...
        List<RepositoryPermission> permissions = permissionStorage.findByUsername(
                username
        );
        return Response.ok(
                permissions
    ...
    @POST
    @Path(
        "/permissions/{username}/{repository}"
    )
    @Consumes(
        MediaType.APPLICATION_JSON
    )
    ...
    public Response setUserPermission(
            @PathParam(
                "username"
            )
            String username,
        ...
        RepositoryPermission permission = permissionStorage.findByUsernameAndRepository(
                username,
                repository
        );
        if (permission == null) {
        ...
        log.infov(
                "Permission updated for user {0} on repository {1}: pull={2}, push={3}",
                username,
                repository,
                permission.canPull,
                permission.canPush
    ...
    public Response deleteUserPermission(
            @PathParam(
                "username"
            )
            String username,
        ...
        log.infov(
                "Permission deleted for user {0} on repository {1}",
                username,
                repository
        );
